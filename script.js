...
function initGame() {
    ...
    */
    const selectedCategory = categoryCard.querySelector('.selected');
    if (selectedCategory) {
        url += `&category=${selectedCategory.dataset.category}`;
        categoryCard.classList.add('hidden');
        skeletonCard.classList.remove('hidden');
        getQuestions();
    } else {
        alert("Choose A Category Below!");
    }
}
...
function processQuestions(data) {
    ...
    */
    data.results.forEach(item => {
        const randomIndex = Math.floor((item.incorrect_answers.length+1) * Math.random());
        const questionObj = {text:item.question, level:item.difficulty, answers:item.incorrect_answers, correctAnswer:randomIndex};
        questionObj.answers.splice(randomIndex, 0, item.correct_answer);
...
function submitAnswer() {
    ...
    */
    if (answerSubmitted) {
        // Verify the user's submitted answer
        const isCorrect = answerSubmitted.textContent === correctAnswer.textContent;

        // Update necessary variables
        if (isCorrect) {
            correct += 1;
            score += getScoreForQuestion(questions[counter].level);
        } else {
